def ensure_role_access_schema_locked(conn):
    ...
    conn.execute("UPDATE role_codes SET updated_at = created_at WHERE TRIM(COALESCE(updated_at, '')) = ''")
    conn.execute("UPDATE role_codes SET status = 'active' WHERE TRIM(COALESCE(status, '')) = ''")
    ...
    conn.execute("UPDATE invite_roles SET updated_at = created_at WHERE TRIM(COALESCE(updated_at, '')) = ''")
    conn.execute("UPDATE invite_roles SET status = 'active' WHERE TRIM(COALESCE(status, '')) = ''")
