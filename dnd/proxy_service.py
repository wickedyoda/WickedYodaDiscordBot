...
def list_proxies(db_path: str, guild_id: int, owner_id: int) -> List[dict]:
    with _get_conn(db_path) as conn:
        rows = conn.execute("SELECT * FROM dnd_proxies WHERE guild_id=? AND owner_id=? ORDER BY id ASC", (guild_id, owner_id)).fetchall()
        return [dict(r) for r in rows]


def delete_proxy(db_path: str, guild_id: int, owner_id: int, name: str) -> bool:
    with _get_conn(db_path) as conn:
        cur = conn.execute("DELETE FROM dnd_proxies WHERE guild_id=? AND owner_id=? AND name=?", (guild_id, owner_id, name))
        conn.commit()
...
def save_proxied_message(db_path: str, message_id: str, proxy_id: int, guild_id: int, channel_id: int, owner_id: int, content: str = "") -> None:
    with _get_conn(db_path) as conn:
        conn.execute(
            ...
            (message_id, proxy_id, guild_id, channel_id, owner_id, content, _utc_now()),
        )
        conn.commit()
