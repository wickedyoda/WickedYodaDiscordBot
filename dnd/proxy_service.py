...
def create_proxy(
    ...
    row = (
        _get_conn(db_path)
        .execute("SELECT * FROM dnd_proxies WHERE guild_id=? AND owner_id=? AND name=?", (int(guild_id), int(owner_id), name))
        .fetchone()
    )
    return dict(row)
...
def get_proxy(db_path: str, guild_id: int, owner_id: int, name: str) -> dict | None:
    with _get_conn(db_path) as conn:
        row = conn.execute(
            "SELECT * FROM dnd_proxies WHERE guild_id=? AND owner_id=? AND name=?", (int(guild_id), int(owner_id), name)
        ).fetchone()
...
def list_proxies(db_path: str, guild_id: int, owner_id: int) -> list[dict]:
    with _get_conn(db_path) as conn:
        rows = conn.execute(
            "SELECT * FROM dnd_proxies WHERE guild_id=? AND owner_id=? ORDER BY id ASC", (int(guild_id), int(owner_id))
        ).fetchall()
...
def save_proxied_message(
    db_path: str, message_id: str, proxy_id: int, guild_id: int, channel_id: int, owner_id: int, content: str = ""
) -> None:
    with _get_conn(db_path) as conn:
        conn.execute(
